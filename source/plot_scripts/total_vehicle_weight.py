...
PLOT_FILE_NAMES = (
    lambda plot, location, n_bins: (
        f"Totalvekter, {location}, {plot}"
        f"{f', {n_bins} intervaller' if n_bins else ''}"
    )
)
...
def plot_bars(df: pd.DataFrame) -> None:
    ...
    for location in locations:
        ...
        for n_bins in N_BINS:
            ...
            binned_values = df_location[weight_columns].T.groupby(
                np.arange(len(weight_columns)) // bin_size
            ).sum().T
            ...
            binned_values.mean().plot(kind='bar', color='blue', width=0.8)
            plt.title(title)
...
def plot_lines(df: pd.DataFrame) -> None:
    ...
    for location in locations:
        df_location = df[df['location'] == location]
        df_location_rel = df_location[weight_columns].div(df_location[weight_columns].sum(axis=1), axis=0)

        plt.figure(figsize=(12, 6))
        for _, row in df_location_rel.iterrows():
            cutoff_idx = np.where(row >= 0.01)[0][-1] if np.any(row >= 0.01) else 0
            cutoff_midpoint = max(cutoff_midpoint or 0, midpoint_labels[cutoff_idx])
            plt.plot(midpoint_labels[:cutoff_idx + 1], row[:cutoff_idx + 1], alpha=0.5)

        title = PLOT_FILE_NAMES('lineplot', location, None)
        plt.title(title)
        plt.xlabel(LABEL_X_AXIS_LINEPLOT)
        plt.ylabel(LABEL_Y_AXIS_LINEPLT)
        plt.xticks(ticks=np.arange(5000, cutoff_midpoint + 1, 5000),
    ...
    for location in locations:
        ...
        cutoff_idx = np.where(avg_relative >= 0.01)[0][-1] if np.any(avg_relative >= 0.01) else 0
        cutoff_midpoint = max(cutoff_midpoint, midpoint_labels[cutoff_idx])

        plt.plot(midpoint_labels[:cutoff_idx + 1], avg_relative[:cutoff_idx + 1], label=f'{location}')
