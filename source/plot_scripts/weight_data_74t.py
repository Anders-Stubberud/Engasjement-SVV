...
def get_datasets():
    ...
    weights_60 = weights[df_74t_vehicle_weight_only_trucks["ekvipasje"] == '3-akslet trekkvogn med 4-akslet tilhenger']
    weights_65 = weights[df_74t_vehicle_weight_only_trucks["ekvipasje"] == '3-akslet trekkvogn med 5-akslet tilhenger']
    weights_68 = weights[df_74t_vehicle_weight_only_trucks["ekvipasje"] == '4-akslet trekkvogn med 4-akslet tilhenger']
    weights_74 = weights[df_74t_vehicle_weight_only_trucks["ekvipasje"] == '4-akslet trekkvogn med 5-akslet tilhenger']
...
def plot_histogram(data, bins, color, edgecolor, title, xlabel, ylabel, save=False, plot=False, save_dir=None):
...
def plot_grouped_histograms(datasets, bins, bar_width, title, xlabel, ylabel, save=False, plot=False, save_dir=None):
    ...
    for i, (_, df, label, color) in enumerate(datasets):
        ...
        plt.bar(x_positions + (i * bar_width), counts, width=bar_width, color=color, edgecolor="black", alpha=0.7, label=label)
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.legend()
...
def main():
    ...
    plot_histogram(weights, bins=100, color="blue", edgecolor="black", title=title, xlabel="Max vekt under hver registrerte kjøretur (KG)", ylabel="Antall", save=save, plot=plot, save_dir=VEHICLE_WEIGHT_FIGURES_DIR_74T)
